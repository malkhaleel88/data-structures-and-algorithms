...
const toLastNames = (people) => {
  let newPeople = [];
  people.map((item, i) => {
    newPeople[i] = `${item.firstName} ${item.lastName}`;
  });
  return newPeople;
};
...
const addPurchases = (arr) => {
  let totalPrice = arr.reduce((accumulator, item) => {
    accumulator = accumulator + item.purchasePrice;
    return accumulator;
  }, 0);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let num = arr.reduce((accumulator, value) => {
...
const returnNames = (arr) => {
  let nameArr = arr.reduce((accumulator, item, i) => {
...
const reversedString = (str) => {
  ...
  let reverse = arrayStr.reduce((accumulator, item) => {
