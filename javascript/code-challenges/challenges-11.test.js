...
function transformToLis(obj) {
  let newArr = Object.entries(obj).map(entry => {
    return `<li>${entry.join(': ')}</li>`;
  });
  return newArr;
}
...
const count = (target, input) => {
  ...
  let newArr = firstArr.filter(value => value === target);
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArray = input.map((item) => {
    if (item) {
      let arr = item.filter((n) => typeof n == "number" && n % 5 == 0);
      return arr.map((m) => {
        return Math.pow(2, m);
      });
    } else if (!item) {
...
let findMaleAndFemale = (data) => {
  ...
  let newNamesArr = data.filter((obj) => {
    if (obj.gender === 'male' || obj.gender === 'female') {
      return obj;
    }
  });

  let names = newNamesArr.map((item) => {
    return item.name;
  });
...
let findShortest = (data) => {
  let sortArr = data.sort((a, b) => a.height - b.height);
